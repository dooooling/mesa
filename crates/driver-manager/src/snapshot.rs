...
/// P1 点元数据：(point_key, source_label)，与 keys 同步 replace。
/// 热路径 apply_batch 一次读锁同时取 key+label，不新增锁（50K/s 锁争用敏感）。
...
pub struct Snapshot {
    ...
    labels: RwLock<HashMap<(String, u32), PointMeta>>,
...
impl Snapshot {
    ...
    pub fn register_points(&self, endpoint_id: &str, defs: &[mesa_core_types::PointDefinition]) {
        ...
        labels.retain(|(ep, _), _| ep != endpoint_id);
        for d in defs {
            labels.insert(
    ...
    pub fn apply_batch(&self, batch: &mesa_core_types::DataBatch, endpoint_id: &str) {
        ...
        // 预构建本批次所需的 key+label 映射快照（仅一次读锁；labels 与 keys
        // 同步 replace，此处不新增锁）。
        let keys_snapshot: HashMap<(String, u32), (String, Option<String>)> = {
            ...
            batch
                ...
                .map(|pv| {
                    ...
                    let v = labels.get(&k).cloned().unwrap_or_default();
                    (k, v)
                })
                .collect()
        };
        ...
        for pv in &batch.values {
            let k = (endpoint_id.to_string(), pv.point_id);
            let (point_key, source_label) = keys_snapshot.get(&k).cloned().unwrap_or_default();
