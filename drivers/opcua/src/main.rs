//! OPC UA Driver 进程入口（方案 §14）。
//!
//! 序列与 s7/focas2/simulator 保持一致：读 token → liveness 守护 → 监听 loopback 端口 → SDK serve。
...
fn main() {
    let port: u16 = match std::env::args().nth(2).or_else(|| std::env::args().nth(1)) {
        ...
        None => panic!("usage: forgelink-driver-opcua --port <u16>"),
    };
    ...
    rt.block_on(async move {
        ...
        if let Err(e) =
            forgelink_driver_sdk::serve(forgelink_driver_opcua::OpcUaDriver, listener, session_token, shutdown).await
        {
            eprintln!("opcua driver exited with error: {e}");
            std::process::exit(1);
        }
    });
}
