...
- **性能预算即验收条件**（§22）：≥50K Point Updates/s 持续 60 分钟；IPC delivery p95 ≤ 20ms / p99 ≤ 50ms（单调时钟测量）；RSS（Core/Test 进程，当前未覆盖独立 Driver 子进程全量）禁止无界增长（60 分钟增幅 ≤10%）；Core 消费降至 25% 时背压仍有效；Conn-1000 无 Task/Handle 泄漏。真实设备不强行对齐 Simulator 指标
